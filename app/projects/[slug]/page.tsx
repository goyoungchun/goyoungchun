...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main>
      ...
      <section className="py-8 md:py-10"><Container className="grid gap-6 lg:grid-cols-3"><div className="rounded-[2rem] border border-border/80 bg-white/88 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.04)] md:p-8 lg:col-span-2"><SectionHeading title="현재 준비 상태" description="스토어 제출과 출시 준비를 위해 필요한 기본 정보를 정리했습니다." /><div className="grid gap-4 md:grid-cols-3 text-sm text-muted"><div className="rounded-2xl border border-border/70 bg-surface p-4"><p className="font-medium text-foreground">출시 상태</p><p className="mt-2">개발중</p></div><div className="rounded-2xl border border-border/70 bg-surface p-4"><p className="font-medium text-foreground">지원 플랫폼</p><p className="mt-2">iOS / Android</p></div><div className="rounded-2xl border border-border/70 bg-surface p-4"><p className="font-medium text-foreground">로그인 방식</p><p className="mt-2">카카오 / 구글 / 애플 / 이메일</p></div></div></div><div className="rounded-[2rem] border border-border/80 bg-white/88 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.04)] md:p-8"><SectionHeading title="정책 및 문의" description="제출과 운영에 필요한 정보를 바로 확인할 수 있습니다." /><div className="flex flex-col gap-3">{policy ? <ButtonLink href={`/privacy/${policy.slug}`}>개인정보처리방침 보기</ButtonLink> : null}<ButtonLink href="/contact" variant="secondary">문의하기</ButtonLink></div></div></Container></section>
      <SiteFooter />
    </main>
  )
}
